...
const job = (evidence: ExecutionJob["evidence"]): ExecutionJob => ({
  id: "1",
  kind: "git_action",
  payload: { action: "push" },
  status: "INTERRUPTED",
...
describe("execution evidence", () => {
  it("summarizes refs without exposing credentials or full paths", () => {
    expect(
      safeGitReference("https://secret@example.com/team/private-repo.git"),
    ).toBe("example.com/…/private-repo");
    expect(safeGitReference("C:\\Users\\name\\project")).toBe("…/project");
    expect(safeGitReference("/home/name/private-project")).toBe(
      "…/private-project",
    );
