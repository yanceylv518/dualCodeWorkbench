  ...
  return items.map((workspace) => ({
    ...
    threads: workspace.threads.map((thread) => ({
      ...
      messages: thread.messages.map((message) => ({
        id: message.id,
        agent:
          message.role as Workspace["threads"][number]["messages"][number]["agent"],
...
export const attachmentContentUrl = (
  workspaceId: string,
  threadId: string,
  attachmentId: string,
) => {
...
export async function provisionWorkspace(value: {
  path: string;
  remote_url: string;
  mode: "init" | "clone";
  name?: string;
}) {
  const r = await fetch(`${API}/workspaces/provision`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function cancelRun(threadId: string) {
  const r = await fetch(`${API}/threads/${threadId}/cancel`, {
    method: "POST",
  });
  if (!r.ok) throw new Error(await r.text());
  return r.json();
...
export async function undoRun(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/runs/${runId}/undo`,
    { method: "POST" },
  );
  if (!r.ok) throw await responseError(r);
  return r.json();
...
export async function fetchThreadDetails(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/details`,
  );
  if (!r.ok) throw new Error(await r.text());
  return r.json();
}
export async function fetchContract(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/contract`,
  );
  if (!r.ok) throw await responseError(r);
  return r.json();
}
export async function saveGovernance(
  workspaceId: string,
  value: ProjectContract["governance"],
): Promise<void> {
  const r = await fetch(`${API}/workspaces/${workspaceId}/governance`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(value),
  });
...
export async function saveTaskContract(
  workspaceId: string,
  threadId: string,
  value: ProjectContract["task"],
): Promise<void> {
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/contract`,
    {
...
export async function prepareHandoff(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/handoffs`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function sendHandoff(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/handoffs/${handoffId}/send`,
    { method: "POST" },
  );
  if (!r.ok) throw await responseError(r);
}
...
export async function requestGitAction(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/git/actions`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function saveWorkspaceRemote(
  workspaceId: string,
  value: { remote_url: string; vps_repo_path: string },
) {
  const r = await fetch(`${API}/workspaces/${workspaceId}/remote`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export async function requestTestRun(workspaceId: string, threadId: string) {
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/tests/run`,
    { method: "POST" },
  );
  if (!r.ok) throw new Error(await r.text());
  return r.json();
...
export async function fetchExecutionJobs(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/jobs`,
  );
  if (!r.ok) throw new Error(await r.text());
  return r.json();
}
export async function retryExecutionJob(
  ...
  const r = await fetch(
    `${API}/workspaces/${workspaceId}/threads/${threadId}/jobs/${jobId}/retry`,
    { method: "POST" },
  );
  if (!r.ok) throw new Error(await r.text());
  await r.json();
