...
afterEach(() => {
  cleanup();
  useStore.setState({ backend: "connecting", workspaces: [], workspaceId: "", threadId: "" });
});
...
describe("workbench", () => {
  ...
  it("shows a real offline state and disables message actions", () => {
    useStore.setState({
      ...
      threadId: "thread-1",
      workspaces: [{ id: "workspace-1", name: "Project", path: "D:/Project", threads: [{ id: "thread-1", title: "Task", state: "CREATED", messages: [] }] }],
    });

    render(<App />);

    expect(screen.getByText("当前无法发送消息或上传附件，请重试连接。")).toBeTruthy();
    expect(screen.getByPlaceholderText("后端离线，请重试连接…")).toHaveProperty("disabled", true);
