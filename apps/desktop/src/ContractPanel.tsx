...
export function ContractPanel({
  ...
  const listField = (
    key: keyof Pick<
      ProjectContract["task"],
      "non_goals" | "acceptance" | "constraints" | "risks"
    >,
    label: string,
  ) => (
    <label>
  ...
  return (
    <div className="contract-panel">
      <header>
        <div>
          <strong>规则与交付契约</strong>
          <span>这些约束会自动提供给 Codex 和 Claude。</span>
        </div>
      ...
      <section
        className={`contract-gate ${value.gate.ready_for_implementation ? "ready" : "missing"}`}
      >
        <Check size={14} />
        <div>
          <strong>
            {value.gate.ready_for_implementation
              ? "已具备正式实施条件"
              : "实施契约尚不完整"}
          </strong>
          <span>
            {value.gate.ready_for_implementation
              ? "产品目标、任务目标、验收标准和产品级原则已定义。"
              : `缺少：${value.gate.missing.join("、")}`}
          </span>
        </div>
      </section>
      <section>
        ...
        <label>
          <span>产品目标</span>
          <textarea
            value={value.governance.product_goal}
            onChange={(event) =>
              setValue({
                ...value,
                governance: {
                  ...value.governance,
                  product_goal: event.target.value,
                },
              })
            }
          />
        ...
        <label>
          <span>产品边界</span>
          <textarea
            value={value.governance.product_boundary}
            onChange={(event) =>
              setValue({
                ...value,
                governance: {
                  ...value.governance,
                  product_boundary: event.target.value,
                },
              })
            }
          />
      ...
      <section>
        ...
        <label>
          <span>任务目标</span>
          <textarea
            value={value.task.goal}
            onChange={(event) =>
              setValue({
                ...value,
                task: { ...value.task, goal: event.target.value },
              })
            }
          />
        </label>
            ...
            onChange={(event) =>
              setValue({
                ...
                task: {
                  ...value.task,
                  status: event.target
                    .value as ProjectContract["task"]["status"],
                },
              })
            }
          >
        ...
        {listField("non_goals", "明确不做")}
        {listField("acceptance", "验收标准")}
        {listField("constraints", "任务约束")}
        {listField("risks", "已知风险")}
      </section>
