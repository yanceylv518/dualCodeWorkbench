...
- 轻量路由不绕过现有审批：命令是否需要授权仍由操作风险决定，流程复杂度与权限边界保持独立。
- 后续模型自动选择按“轻量 / 标准 / 高级”三档实现：简单问答与只读命令使用轻量模型或低推理强度，普通开发使用默认模型，架构、安全、迁移及大范围改动使用高级模型和双 Agent；模型不可用时必须显示回退原因，不允许静默替换。
- 功能规划、产品规划、需求规划、产品/功能路线图和功能优先级由 Claude 主导澄清与规划，再由 Codex 做可实现性复核或进入实现；此类任务不得按普通功能开发直接交给 Codex 起步。

## 2026-08-02 智能协作消息级分流

- 智能协作不再把每条未命中关键词的消息都视为新开发任务；“阅读项目、分析现状、只做分析、不修改代码、继续、接下来”等只读分析、追问和延续指令直接交给单 Agent 连续处理。
- 明确的功能开发、架构、高风险、构建和产品设计任务仍进入 Codex 实现、验证、Claude 审查与整改闭环；单 Agent 产生较大 Diff 时继续保留事后升级审查机制。
- 路由回归覆盖只读分析、继续追问及真实开发任务，避免对话式消息反复显示“智能协作·第 1/3 轮”。

## 2026-08-02 任务标题自动命名

- “新开发任务”只作为空任务在收到第一条需求前的临时占位；第一条用户消息入库时，后端会用规范化后的需求摘要生成并持久化任务标题，前端直接使用后端返回值，不再依赖额外的异步重命名请求。
- 项目列表加载时会自动修复已有对话但仍保留默认名称的历史任务，使用首条有效用户消息生成标题；用户手动修改过的标题不会被覆盖。
- 后端 API 集成 22 项、前端 store 25 项、TypeScript 与 Ruff 验证通过。

## 2026-08-02 智能协作审查协议自愈与消息归类

- `review.v1` finding 的数字行号会自动归一化为字符串，兼容 Claude 常见的 `"line": 3` 输出，不再误报 `schema_mismatch`。
- 审查协议解析失败会在同一快照上自动要求 Claude 重发一次；二次失败进入明确的系统阻塞状态，不再错误地要求用户补充任务目标或任务契约。
- 智能协作中的 Claude 审查改为内部协议调用：工具活动继续进入折叠过程，原始思考正文与 JSON 不再保存到聊天消息；解析成功后只展示中文、结构化的审查结论。
- 审查提示明确要求标题、正文、摘要和 finding 全部使用中文；协议与编排专项 32 项及 Ruff 已通过。

## 2026-08-02 智能协作任务契约自动草拟

- 智能协作会直接使用用户原始需求作为任务目标，并按任务分类自动生成保守、可验证的验收草案；目标明确时不再因空验收标准进入 `WAITING_USER`。
- 修复桌面端真实消息入口遗漏：智能路由调度器现在会在创建协作运行前，以原始消息建立并显式落库任务契约；不再只有独立 collaboration API 生效。
- 只有无法识别任务目标时才暂停询问，提示用户用自然语言说明希望完成的结果，不再要求理解或手填任务契约格式。
- 既有等待任务选择“补充后继续”时会自动修复缺失契约；时间线明确说明无需固定格式，并提供自然语言示例。
- 第一轮前端全量 97 项及 TypeScript、ESLint、Prettier 通过；真实消息入口修复后的智能路由、编排和 API 专项 38 项、后端全量 278 项及 Ruff 通过。Windows sidecar 与隔离目录 Tauri release 构建通过，新免安装程序 SHA-256 为 `9AC65DA80E4651C65A29CDF3A2F09B16D9746ADED128083BD1416708A7586DC8`。
...
- 目标 turn 已完成但实时 `turn/completed` 丢失时，补齐尚未收到的回答文本并合成成功终态；已经流式收到的文本不会重复显示，工作台不再把 Codex 中已完成的线程误报为失败。
- 只有目标 turn 仍在执行、明确失败或无法确认时才进入现有中断与安全阻塞路径；存在副作用的操作仍不自动重放。

## 2026-08-01 Codex 失联会话重建

- Codex 命令活动连续 180 秒没有任何协议事件时进入探活与中断流程，不再让明确失活的命令等待十分钟后才反馈。
- 无进展 turn 即使 `thread/read` 曾短暂成功，也会关闭整棵 app-server 进程树；下一次恢复会启动干净的协议进程，避免继续复用已经丢失完成信号的传输连接。
- 新 app-server 进程复用历史会话前会显式调用 `thread/resume`，不再把旧 thread id 直接交给一个尚未加载该线程的新进程。
- 仍保留副作用安全边界：存在活动命令或文件操作时不自动重放失败轮次，由用户确认后恢复。

## 2026-08-01 Codex 活动终态与无进展回收修复

- 兼容 Codex app-server 以 `item/updated` 携带终态的协议形式，后端活动跟踪和前端工具摘要都会立即结束对应命令，不再长期显示旋转状态。
- watchdog 探活只用于确认传输仍存活，不再把“进程可连接”误判为“任务有进展”；超过无进展窗口后会等待一次短暂宽限、主动中断并返回明确错误，避免持续续命卡住。
- 回归验证覆盖后端 app-server 14 项及前端 store 25 项；后端全量 269 项、TypeScript、ESLint 与 Ruff 均通过。前端全量另有既存 StylePolicy 最小字号失败（`ContractPanel.css` 的 `font: 11px/1.5 sans-serif` 被当前正则误报），与本次活动修复无关。

## 2026-08-01 仓库访问配置 R1-R3

- R1：新增本机与 VPS 的独立只读仓库访问检测，统一识别 SSH/HTTPS，并将仓库不存在、公钥未授权、认证缺失、主机指纹失败及网络错误转换为可行动状态；检测不会尝试写入仓库，写权限保留为首次推送时确认。
- R2：右侧 VPS 仓库卡片新增可折叠“仓库访问配置”，展示本机/VPS 的读取状态、传输类型、原因和重新检测入口；不会再用单一“克隆失败”覆盖凭据问题。
- R3：支持通过既有审批流程在 VPS 为项目生成独立 Ed25519 Git 密钥。私钥只保存在 VPS，应用仅持久化密钥路径、公钥和指纹；界面可复制公钥并在添加 GitHub Deploy key 后复检。clone/fetch/pull 会复用该项目密钥。
- 验证：后端仓库访问/SSH/API 专项 42 项、前端组件与 store 专项 29 项、TypeScript 和 Ruff 全部通过。

## 2026-07-30 智能协作界面开关

- `Agent 与模型` 设置新增“智能协作”实验性开关，配置持久化到本地
  `agent-settings.json`，并删除环境变量双真源。
- 后端能力查询、消息路由、共享记忆、交接、协作运行和任务清理统一读取运行时
  设置，保存后立即生效，无需重启 sidecar。
- 主界面在保存后重新读取 capabilities，即时显示或隐藏智能协作发送模式；
  关闭开关时若当前正处于
  smart 模式，会安全回退到 Codex。
- 前端 92 项、后端 258 项、TypeScript、严格 ESLint 与 Ruff 全部通过；
  C6-3b 已按最新执行清单关闭。

## 2026-07-30 智能协作 C6-1 交接与审查证据视图

- 交接检查器现在识别 `handoff.v2`，按任务契约、仓库快照、diff 统计、测试证据、
  open findings 与风险分区展示，不再暴露裸 JSON；legacy 交接渲染保持兼容。
- findings 视图归入「交接」页，展示严重级别、类型、文件位置、描述、验收标准与
  open/resolved 状态，并提供明确空态。
- 新增 3 项组件测试；前端全量 81 项、TypeScript、严格 ESLint、改动文件
  Prettier、后端 258 项与 Ruff 全部通过。下一项为 C6-2 协作阶段时间线。

## 2026-07-30 智能协作 C5-4 六条端到端验收

- 新增六条从 HTTP API 入口驱动的协作验收，使用可控 Agent 回调和本地裸仓伪
  VPS，覆盖直接通过、一次整改、整改到限、审批续跑、审查中取消清理影子 ref，
  以及应用重启阻塞后恢复且不重放副作用。
- 六条专项、后端全量 258 项、前端 78 项、Ruff 与 TypeScript 全部通过；前端
  零 diff，智能协作开关仍默认关闭。C5 已完成，按执行约定停下等待 Claude review。

## 2026-07-30 智能协作 C5-3 停止条件、控制 API 与事件

- 编排闭环已具备整改/总轮次上限、连续两轮无进展检测，以及 Agent 失活或 VPS
  不可达时的可恢复阻塞；达到限制后保留未解决 findings 并等待用户裁决。
- 新增协作运行创建、当前运行、暂停、恢复、取消、用户决策和 findings API；所有
  run-id 操作校验 workspace/thread 归属，恢复与继续修复会续跑同一持久化运行。
- 九类协作事件已接线且只携带摘要与 ID。专项 33 项、后端全量 252 项、前端
  78 项、Ruff 与 TypeScript 全部通过。下一项为 C5-4 六条 API 驱动 E2E。
...
- 新增默认关闭、可由环境变量开启的智能协作开关；关闭时共享记忆路径零数据库
  访问，现有 Codex/Claude prompt 保持原样。
- 新增 8,000 字符共享记忆预算，按目标/验收、决策、仓库、风险和其余证据排序；
  stale/unverified 优先丢弃，confirmed 永不截断，溢出有明确标记。
- Codex 与 Claude 复用同一注入入口，在现有项目与任务契约之后获得相同的已确认
  目标、当前 commit 和未关闭风险。C1 已全部完成，按约定停在 C2 前等待 review。
...
- 首次 CI 已确认双平台使用 Ruff 0.15.22，Ruff、后端测试、TypeScript、ESLint
  全部通过；按 CI 输出机械格式化 3 个既有漂移文件后再次触发完整流水线。
- GitHub Actions run `30413530551` 的 Windows/Ubuntu 任务均完整通过，包含 Ruff、
  后端 124 项、TypeScript、ESLint、Prettier 和前端 78 项；C-R3、C-R4 已关闭。
...
- 助手回复末尾的复制与重试操作恢复为透明、内容宽度的小图标行，不再被历史 `.message-body > div` 规则误判成正文并渲染为整块有色背景。
- 前端 78 项、TypeScript、严格 ESLint 与 Claude stream 后端 8 项专项测试通过。
...
## 2026-07-30 智能协作 C6-4 构建

- 应用版本由 `0.1.6` 递增至 `0.1.7`，Vite 生产前端、Windows sidecar 与
  Tauri release 构建成功。
- C6-3b-R1 格式修复提交 `9c748c2` 的 GitHub Actions CI（run `30530734664`）
  已在 Ubuntu/Windows 双平台全绿；发布前进一步统一后端包、FastAPI 与 Codex
  app-server 客户端版本为 `0.1.7`。版本一致性提交 `14d414b` 的 CI
  （run `30531780458`）再次双平台全绿，最终安装包从该提交基线重建。
- 已生成 MSI 与 NSIS 安装包；NSIS SHA-256 为
  ...
  旧校验和对应的安装包已作废。
- C6-4 五项真实验收尚未执行，智能协作仍未切换为默认入口。

## 2026-07-30 智能协作任务契约输入框可读性修复

- 为 `WAITING_USER` 决策区输入框补齐暖中性主题样式，显式设置深色背景、正文颜色、
  占位符颜色、插入光标、边框、焦点环及禁用态，避免浏览器默认白底与浅色文字叠加。
- 前端全量 92 项测试、TypeScript 类型检查、严格 ESLint 与 Prettier 通过。

## 2026-07-30 智能协作决策请求修复

- 修复安装版在 `WAITING_USER` 中点击“调整后重入 / 直接整改 / 停止”时浏览器预检失败：
...
## 2026-07-30 智能协作决策传输加固与 0.1.8 构建

- 实机确认 0.1.7 sidecar 已包含 CORS 白名单且预检返回 200，但 WebView 的协作操作
  仍可能以 `TypeError: Failed to fetch` 结束；为消除额外预检依赖，前端改用查询参数传递
  workspace/thread 上下文，后端同时兼容查询参数与旧请求头，令牌鉴权保持不变。
- 前端 92 项、后端 259 项、TypeScript、严格 ESLint、Ruff 与生产构建通过。
- 版本递增至 `0.1.8` 并重新生成 Windows sidecar、主程序、MSI 与 NSIS 安装包。
- 独立主程序 SHA-256：
  `2DF6A6D0002F0F2F98493C7F4053A8072F805A6A8A513DFA592D253C2613886F`。
- NSIS 安装包 SHA-256：
  `3FB64D9ED9A8E68E2592A77F6F4751039E70658087C56D9DE6541D8E5586000D`。
- MSI 安装包 SHA-256：
  `82475B621C829720581F06E42ADEAF0ED4437482288C0A6735A7C735E4E612D2`。

## 2026-07-31 Codex 长任务看门狗与可恢复性加固

- 将固定 60 秒无事件即杀连接改为活动感知看门狗：普通静默 120 秒后进行
  `thread/read` 探活并给予 15 秒宽限；已有命令执行事件时使用 600 秒窗口，
  探活成功则继续等待，不再把安静运行的长命令误判为 app-server 死亡。
- 超时失败现在记录最后协议方法、活动工具、进程号、空闲时长、探活结果、
  脱敏 stderr 和 `retry_safe` 到 `agent_runs.failure_context`，同时写入审计日志，
  后续可以从数据库直接定位真实停滞场景。
- 不确定是否已有命令或文件副作用的超时不再自动用新会话重放请求，避免重复执行；
  无活动且探活成功时先中断失活 turn，探活失败才重置整个 app-server。
...
- 未选择工作区时，右侧检查器不再把 `gitStatus === undefined` 误解为正在加载，
  也不再渲染 VPS 仓库默认表单、测试配置、规则或交接面板。
- 仓库、变更和运行日志子导航仅在项目存在时出现；空项目态统一提示先打开或创建
  Git 项目，避免用户误以为应用仍在读取一个不存在的仓库。
- 新增空工作区回归用例；前端 93 项测试、TypeScript 与严格 ESLint 全部通过。

## 2026-07-31 左侧项目导航与实时状态修复

- 左侧项目导航改为紧凑列表层级：任务降低卡片感和纵向占用，项目标题行直接提供“新建任务”入口，不再把创建入口混入任务列表。
- 任务菜单补齐点击空白处、按 Esc、切换任务三类关闭路径，避免菜单打开后无法收起。
- WebSocket 错误处理增加当前项目与任务校验；删除最后一个项目或清空选择后统一进入 disconnected，旧连接回调不再把空工作台误标为“实时连接重连中”。
- 新增项目级创建入口、任务菜单收起和旧连接错误隔离回归用例；前端专项 49 项、后端 260 项、TypeScript 检查通过。
