...
### C6-4-R1 复验（2026-08-01，Claude）

**结论：读取通道修复本身通过；「CI 双平台绿」验收项被验收调试期批量提交的
格式债阻塞，立 C6-4-R2 关闭后一并放行。**

- 根因三处修复逐项到位 ✓：`limit=10MiB` 常量与注释；`readline()` 超限
  容错——丢弃超限行、发终端诊断、继续读取（超限行残尾会因 JSON 解析失败
  自然跳过，重同步正确）；`finally` 重构为先广播 transport 错误、再限时
  终止进程（3 秒 wait 后 kill），挂死路径消除，`CancelledError` 正确透传。
- 补救层超出返工要求且方向正确 ✓（`cacfa83`）：看门狗超时探活升级为
  `thread/read` 轮次快照——轮已 completed 时恢复未送达的文本后缀并以
  `recovered=true` 正常收官，不再把已完成的轮误判为失败；failed/
  interrupted 明确报错。前缀比对防重复有专项测试。
- 新回归测试四条与要求一一对应 ✓：>64KiB 行解析、读取协程死亡限时通知
  不挂死、恢复不重复已流式文本、完成事件丢失后恢复。
- 独立复验：后端全量 276 项、Ruff 通过。
- CI ✗（无关阻塞）：双平台 Prettier 失败——真实验收调试期间直接推送的
  UI 批量提交（`4fd7909` 起）遗留 6 个未格式化文件（`api.ts`、`App.tsx`、
  `RemoteRepository.tsx/.test.tsx`、`index.css`、`store.ts`），CI 自
  `4fd7909` 起连红，本修复的「CI 双平台绿」验收项被挡。

**返工项：**

- **C6-4-R2｜恢复 CI 绿并重建安装包（一个 commit + 产物）。**
  1. 对上述 6 个文件执行 Prettier 机械格式化，不改逻辑。
  2. 推送确认 CI 双平台绿后，重建安装包（含 C6-4-R1 修复），更新 SHA-256，
     旧校验和标注作废；用户凭新包复测大输出命令轮次。
  3. 流程加固（写入执行约定备忘）：任何直接推送（含验收调试期的快速
     修复）前本地必须跑
     `pnpm --filter @dualcode/desktop exec prettier --check "src/**/*.{ts,tsx,css}"`
     ——这是第三次同类 CI 阻断（前两次：C-R4 期间与 C6-3b-R1）。
  - **验收**：CI 双平台绿；新 SHA-256 已记录；备忘写入。
