...
- [x] `apps/backend/dualcode/security.py`：扩充规则至少覆盖 `.env*`、`*.pem/.key/.p12/.pfx`、`id_rsa*/id_ed25519*/id_ecdsa*`、`credentials.json`、`.npmrc`、`.netrc`、`*.keystore`；用可维护的规则列表（glob + 说明）替代散落条件。
- [x] 补足单元测试：上述每类至少一条正例一条反例。
- **验收**：`test_security.py` 覆盖新规则并通过。
- **验证结果（2026-07-13）**：安全规则专项 22 项、后端全量 90 项及 Ruff 检查通过；规则集中为带中文原因的不可变 glob 列表，并覆盖敏感正例与相邻非敏感反例。

**Phase 0 最终验收（2026-07-13）**：后端 90 项、前端 7 项、E2E 1 项、Ruff、TypeScript、
ESLint（0 error）、Prettier 与 Rust `cargo check` 全部通过。Phase 0 到此停止，等待 Claude review。
...
- [x] `App.tsx:58-63`：改为「仅当用户位于底部附近（阈值 ~80px）时跟随滚动」；用户上翻后出现「回到最新」悬浮按钮，点击回底。
- [x] 新增组件测试覆盖"上翻时不强制吸底"。
...
- [x] 按统一 Diff 的文件边界拆分 diff：文件列表 + 单文件只读视图。
- [x] Monaco 改为动态 `import()` 懒加载，消除生产构建 chunk 警告（PROJECT_STATUS 已知缺口 4）。
- **验收**：`pnpm build` 无 chunk 大小警告；多文件修改时可按文件查看。
- **验证结果（2026-07-13）**：Diff 面板支持多文件导航并保留只读 Monaco；编辑器和 Monaco 运行时仅在进入变更页后加载，初始 JS 降至 459kB，生产构建无 chunk 警告。TypeScript、前端 20 项、严格 ESLint 与 Prettier 通过。
...
- [x] `store.error` 单字符串改为通知队列（id、级别 error/info、文案、可选自动消失时长）；错误默认不自动消失、信息类 5s 自动消失；可堆叠展示（右下角）。
- [x] 后台轮询失败（如"无法刷新恢复任务"）降级为 info 且自动消失，不打断用户。
- [x] 相应更新 `store.test.ts`。
- **验证结果（2026-07-13）**：通知支持堆叠、分级、逐条关闭和信息类自动消失，VPS/恢复后台刷新失败不再覆盖用户错误；TypeScript、前端 22 项、严格 ESLint 与 Prettier 通过。

### P2-6 任务管理补全

- [x] 线程重命名：任务标题处双击或菜单进入行内编辑并持久化，后端提供 PATCH 接口与审计。
- [x] 首轮用户消息发送后，若标题仍为默认「新开发任务」，自动以消息前 20 字生成标题，不调用模型。
- [x] 线程删除：菜单入口 + 二次确认，后端阻止删除运行中任务，并级联清理该线程数据、附件文件和审计。
- **验证结果（2026-07-13）**：任务支持自动命名、行内重命名和显式确认删除；后端全量 94 项、前端 24 项、Ruff、TypeScript、严格 ESLint 与 Prettier 通过。
