...
- [ ] 新建 `components/CodeBlock.tsx`，接管 `MarkdownMessage.tsx` 中的 `pre/code` 渲染。
- [ ] 语法高亮：用 `highlight.js` **按需注册**常用语言（ts/js/tsx/python/rust/bash/json/diff/css/html/sql/yaml/markdown 即可，禁止全量引入）；未识别语言回退纯文本。高亮主题写成本项目自己的 CSS（对齐现有深色配色），不要引入整套第三方主题文件。
- [ ] 语言标签渲染在代码块左上角（`.message-code` 现有 27px 顶部 padding 就是为它预留的）。
- [ ] 复制按钮位于代码块右上角，hover 时显现，点击复制原始代码文本并给出瞬时反馈（按钮文案切换"已复制"约 1.5s，不要用全局通知刷屏）。
- [ ] 超过约 400 行的代码块默认折叠展示前若干行 + "展开全部 (N 行)"。
...
- [ ] Agent 消息（codex/claude）hover 时在消息卡右上角显示工具条：**复制**（复制 Markdown 原文）。触屏/键盘可达性：工具条按钮需可聚焦，focus 时同样显现。
- [ ] 用户消息现有的两个**常驻**按钮（"编辑后重新发送"、"重试本轮"，App.tsx MessageCard）改为同样的 hover 工具条样式，行为不变。
- [ ] 复制成功给瞬时反馈（同 U1 的就地反馈方式）。
...
- [ ] `MessageCard`（App.tsx）：当消息 id 为 `stream-` 前缀（进行中的流式占位，见 store.ts `stream-${run_id}`）时，渲染**纯文本 + 尾部光标**（复用 `.thinking-block` 的光标动画样式，pre-wrap），不经过 ReactMarkdown。
- [ ] `message.created` 将占位替换为持久消息后（id 更换），自动切换为 MarkdownMessage 渲染，无需额外状态。
- [ ] 保持"接近底部才跟随滚动"的现有行为不受影响。
