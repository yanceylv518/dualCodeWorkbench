...
- [x] store（store.ts `toolStep` / delta 合并处）为 reasoning 步骤记录 `startedAt`，在步骤离开 running 状态时记录 `completedAt`（`ActivityStep` 增加这两个可选字段）。
- [x] `ActivityCard`（App.tsx）：思考块结束后收起为一行"已思考 X 秒"胶囊（样式对齐 Claude App 的 thought pill），点击展开回看完整思考文本；运行中保持现有流式展示。
- [x] 活动卡整体收起逻辑不变（终态默认折叠）。
- **为什么**：当前思考结束后过程混在活动时间线里，没有独立的时长入口；"已思考 X 秒"是 Claude App 标志性的完成态。
- **验收**：store 测试覆盖 reasoning 步骤时间戳记录；组件测试覆盖完成态胶囊显示时长、点击展开显示全文。
- **验证结果（2026-07-14，Claude 执行）**：`stampStep` 统一保留最早开始时间并在离开 running 时记录结束时间，错误/取消路径（`settleActivity`）与整体完成路径同样打点；完成态渲染为可展开的 `thought-pill`（「已思考 X 秒」），运行中流式展示不变。前端 62 项、TypeScript、严格 ESLint 通过。

### U5 视觉降噪：边框减量、字号、圆角与过渡 token

- [x] 消息正文字号 `--text-lg` 14px → 新增 `--text-reading: 15px` 用于 `.message-content`（其余 UI 字号不动）。
- [x] 削减 1px 边框的使用：消息区、检查器分区、卡片等改为**背景明度分层**（建立 2-3 级 surface 变量，如 `--surface-0/1/2`），仅保留必要的输入框和交互边框。逐块替换，禁止一次性全局查换。
- [x] 统一圆角 token：`--radius-sm: 6px / --radius-md: 10px / --radius-lg: 14px`，替换现有 4-16px 混用值。
- [x] 建立 `--transition-fast: 150ms ease` 并应用到：消息浮现（新消息淡入+2px 上移）、右侧面板 tab 切换、hover 态。`prefers-reduced-motion` 时禁用位移动画。
...
- **验证结果（2026-07-14，Claude 执行）**：token（reading/surface/radius/transition）集中定义于 :root，应用收敛为文件末尾单一「U5 视觉降噪层」并注明后续视觉调整只改该层；检查器分区改为 surface 软卡片、测试卡与侧栏页脚去边框；消息/系统事件/活动卡增加 150ms 浮现动效并尊重 reduced-motion；顺带消除 6 处用 `font:` 简写绕过字号策略的 9-10px 文本，策略测试补堵该漏洞并断言全部新 token。前端 63 项、TypeScript、严格 ESLint、生产构建通过。视觉效果待用户截图确认，不满意可整体 revert 本 commit。

### U6 细节小件

- [x] 附件托盘（Composer 的 `.attachment-chip`）：图片附件显示缩略图预览（复用 `attachmentContentUrl`），非图片保持现有图标。
- [x] "回到最新"悬浮按钮：用户上翻期间有新消息到达时显示计数（"回到最新 · N 条新消息"），回底后清零。
- [x] 消息时间戳改为 hover 显现（布局占位保留，避免抖动）。
- **验收**：组件测试覆盖缩略图渲染、新消息计数增减、时间戳 hover 显隐。
- **验证结果（2026-07-14，Claude 执行）**：图片附件在托盘显示 26px 缩略图、非图片保持图标，移除按钮补充可访问名称；上翻期间新消息计数累加、回底清零（修复了函数式更新晚于 ref 改写导致增量为 0 的时序 bug）；时间戳 hover/focus 显现、触屏常显，占位保留无抖动。前端 66 项、TypeScript、严格 ESLint 通过。
...
- 2026-07-14 安装包验收后，用户明确要求移除 Agent 消息右上角的整条消息复制入口；U2 的用户消息编辑/重试工具条和 U1 的代码块复制保持不变。该产品决策覆盖 U2 中 Agent 整条消息复制的原设计。
